...
contract FairDataManagerDeprecated is ChainlinkClient, ConfirmedOwner {
    ...
    function makeRequest(address _to) public payable{
        ...
        (bool payment, ) = address(this).call{value: msg.value}(""); //TODO improve: allow smart contract to send _amount either via approval function
        require(payment == true, "No personal data is for free!");

        //transparently record request
        requests[requestCount] = Request(msg.sender, _to, msg.value * 999 / 1000, false); //smart contract keeps 0.1% of amound data seekers offers for data
    ...
    function approveRequest(uint _requestId) public {
        ...
        (bool paymentSuccess, ) = requests[_requestId].from.call{value: requests[_requestId].amount}(""); //payment by smart contract
        require(paymentSuccess == true, "Payment of data owner unsuccessful");
    ...
    //TODO get rid of _address input (only existing due to difficulty of casting address to string)
    function forwardPersonalData(uint _requestId, string memory _address) internal returns (bytes32 requestId) {
        require(requests[_requestId].accepted = true, "Forwarding of personal data not authorized");
    ...
    function fulfillRequestPersonalData (bytes32 _requestId, string memory _name) public recordChainlinkFulfillment(_requestId) {
        emit PersonalData(_name); //TODO how to make sure that only the paying data seekers sees personal data
    }
    ...
    function withdrawEth(uint _amount) public onlyOwner {
        require(payable(this).transfer(msg.sender, _amount), 'Unable to transfer');
    }
}
